  ...
  return (
    <>
      <ConversionHero
        ...
        description="Quer fazer parte da equipe Accesslift? Envie seus dados e curriculo para nosso banco de profissionais."
      />
      <section className="mx-auto grid max-w-4xl gap-6 px-4 py-12 md:px-6 lg:grid-cols-[0.8fr_1.2fr]">
        <div>
          <h2 className="text-slate-950">Cadastre seu curriculo</h2>
          <p className="mt-3 text-sm leading-6 text-slate-600">
            A Accesslift atua no segmento de plataformas elevatorias e reune profissionais em areas relacionadas a operacao, atendimento, logistica, manutencao e administracao. O envio nao representa garantia de contratacao ou participacao imediata em processo seletivo.
          </p>
        </div>
        <form className="grid gap-4 rounded-lg border border-slate-200 bg-white p-5 premium-shadow" onSubmit={submit}>
          ...
          <label className="grid gap-1.5 text-xs font-black uppercase tracking-wider text-slate-600">
            ...
            <select className={inputClasses} value={values.area} onChange={(event) => setValues((current) => ({ ...current, area: event.target.value }))}>
              ...
              <option value="tecnico-manutencao">Tecnico / Manutencao</option>
              <option value="logistica-operacao">Logistica / Operacao</option>
          ...
          <p className="text-xs leading-5 text-slate-500">
            Os dados enviados serao utilizados para analise de oportunidades profissionais e tratamento do contato relacionado ao processo de recrutamento, conforme a Politica de Privacidade da Accesslift.
          </p>
          {status === "error" && (
            <p className="flex items-center gap-2 rounded-md bg-red-50 p-3 text-sm font-semibold text-red-700" role="alert">
              <AlertCircle className="h-5 w-5" aria-hidden />
              Informe nome, e-mail valido, telefone e curriculo para continuar.
            </p>
          )}
          {status === "success" && (
            <p className="flex items-center gap-2 rounded-md bg-lime-50 p-3 text-sm font-semibold text-lime-900" role="status">
              <CheckCircle2 className="h-5 w-5" aria-hidden />
              Curriculo registrado localmente para validacao do fluxo. A transmissao definitiva depende da integracao backend.
            </p>
          )}
