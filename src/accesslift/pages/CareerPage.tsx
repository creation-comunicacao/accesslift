  ...
  return (
    <>
      <ConversionHero
        ...
        title="Trabalhe com a Accesslift"
        description="Canal preparado para receber candidaturas quando a integração com o atendimento de RH for definida."
      />
      <section className="mx-auto grid max-w-4xl gap-6 px-4 py-12 md:px-6 lg:grid-cols-[0.8fr_1.2fr]">
        <div>
          <h2 className="text-slate-950">Envie sua candidatura</h2>
          <p className="mt-3 text-sm leading-6 text-slate-600">
            Informe seus dados e anexe o curriculo. O destino seguro dos dados deve ser conectado ao backend ou canal de RH antes da publicacao.
          </p>
        </div>
        <form className="grid gap-4 rounded-lg border border-slate-200 bg-white p-5 premium-shadow" onSubmit={submit}>
          ...
          <label className="grid gap-1.5 text-xs font-black uppercase tracking-wider text-slate-600">
            Telefone
            <input className={inputClasses} type="tel" autoComplete="tel" value={values.phone} onChange={(event) => setValues((current) => ({ ...current, phone: event.target.value }))} />
          </label>
          <label className="grid gap-1.5 text-xs font-black uppercase tracking-wider text-slate-600">
            Area de interesse
            <input className={inputClasses} value={values.area} onChange={(event) => setValues((current) => ({ ...current, area: event.target.value }))} placeholder="Ex.: Operacoes, comercial ou manutencao" />
          </label>
          <label className="grid gap-1.5 text-xs font-black uppercase tracking-wider text-slate-600">
            Mensagem
          ...
          {status === "error" && (
            <p className="flex items-center gap-2 rounded-md bg-red-50 p-3 text-sm font-semibold text-red-700" role="alert">
              ...
              Informe nome e um e-mail valido para continuar.
            </p>
          )}
          {status === "success" && (
            <p className="flex items-center gap-2 rounded-md bg-lime-50 p-3 text-sm font-semibold text-lime-900" role="status">
              <CheckCircle2 className="h-5 w-5" aria-hidden />
              Estrutura validada localmente. A transmissao para RH depende da integracao backend.
            </p>
          )}
