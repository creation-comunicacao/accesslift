...
export function SupportRequestForm() {
  ...
  return (
    <form
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          setStatus("success");
          setMessage("Solicitacao enviada para avaliacao da equipe Accesslift.");
    ...
    >
      ...
      <label className={labelClasses}>Cidade *<input className={inputClasses} autoComplete="address-level2" value={values.cidade} onChange={(event) => update("cidade", event.target.value)} placeholder="Cidade da operacao" /></label>
      ...
      <label className={`${labelClasses} md:col-span-2`}>Descricao da ocorrencia *<textarea className={`${inputClasses} min-h-32 py-3`} value={values.descricao} onChange={(event) => update("descricao", event.target.value)} placeholder="Descreva a situacao, local e qualquer informacao que ajude a avaliacao tecnica." /></label>
