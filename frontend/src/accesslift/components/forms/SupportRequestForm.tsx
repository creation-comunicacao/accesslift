  ...
  return (
    <form
      ...
      onSubmit={async (event) => {
        ...
        if (!valid) {
          setStatus("error");
          setMessage("Informe nome, WhatsApp, cidade, descricao da ocorrencia e se ha plataforma Accesslift em locacao.");
          return;
        }
    ...
    >
      ...
      <label className={`${labelClasses} md:col-span-2`}>Descricao da ocorrencia *<textarea className={`${inputClasses} min-h-32 py-3`} value={values.descricao} onChange={(event) => update("descricao", event.target.value)} /></label>
      <fieldset className={`grid gap-3 md:col-span-2 ${labelClasses}`}>
        <legend>Esta com uma plataforma Accesslift em locacao? *</legend>
        <div className="flex flex-wrap gap-4 text-sm font-semibold normal-case tracking-normal text-slate-700">
          <label className="flex items-center gap-2">
            <input
              type="radio"
              name="locacaoAccesslift"
              checked={values.locacaoAccesslift === true}
              onChange={() => update("locacaoAccesslift", true)}
          ...
          <label className="flex items-center gap-2">
            <input
              type="radio"
              name="locacaoAccesslift"
              checked={values.locacaoAccesslift === false}
              onChange={() => update("locacaoAccesslift", false)}
              className="h-4 w-4 accent-lime-500"
            />
            Nao
          </label>
